...
if __name__=='__main__':
	...
	dataDic_all01={};targetDic={}; dataDic_notall01={};dataDic_notall01_noInt={}
	mat=merge4.drop(['id','fault_severity'],axis=1).values;print mat.shape
	allFeaList=list(np.unique(mat.flatten() ) ) 
	feaList_noLocation=[f for f in allFeaList if type(f)==str and 'location' not in f]+[i for i in allFeaList if type(i)!=str]
	feaList_noLocation_noInt=[f for f in allFeaList if type(f)==str and 'location' not in f]
	#print len(allFeaList),len(feaList_noLocation)
	...
	for idi in uniqueId[:]:
		...
		for col in patch.columns:
			fea_xi=fea_xi+list(np.unique(patch[col].values)  )
		#print fea_xi
		# all dummy 01 include location 
		fea_all01=str2dummy(fea_xi,allFeaList,logVolumeDic)#location volumNum all01
		# all dummy 01 except for location	
		fea_notall01=str2dummy(fea_xi,feaList_noLocation,logVolumeDic)
		fea_notall01_noInt=str2dummy(fea_xi,feaList_noLocation_noInt,logVolumeDic)
		loc=[patch['location'].values[0].split(' ')[1] ]
		#print fea_all01[np.nonzero(fea_all01)[0]],fea_all01.shape
		#print fea_notall01[np.nonzero(fea_notall01)[0]],fea_notall01.shape
		#print fea_notall01_noInt[np.nonzero( fea_notall01_noInt)[0]],fea_notall01_noInt.shape
		dataDic_all01[idi]=fea_all01;#print fea01.shape
		dataDic_notall01[idi]=np.concatenate((fea_notall01,np.array(loc) ));
		dataDic_notall01_noInt[idi]=np.concatenate((fea_notall01_noInt,np.array(loc) ));
		targetDic[idi]=target
		 
	#print dataDic_all01.values()[0].shape,dataDic_notall01.values()[0].shape,targetDic,dataDic_all01.values()[0],dataDic_notall01.values()[0]

	save2pickle([dataDic_all01,dataDic_notall01,dataDic_notall01_noInt,targetDic,allFeaList],'dataTarget_FeaAll01_notall01')
