...
if __name__=='__main__':
	...
	#########
	dataDic_resource8,dataDic_resourceRest=load_pickle(dataPath+'dataDic_resource8Rest')#{idi:[]...}
	dataDicCount,targetDic,_=load_pickle(dataPath+'dataTargetFeaAll_count')		     #{idi:arr
	...
	#pick up seve1,combine fea:count,data_notall01

	#merget dic: seve2|seve1 seve345
	#dicMerge=dataDic_seve1.copy()
	#dicMerge.update(dataDic_seve345)
	#
	dataDic_01_count={}
	for idi,xiDic in dataDic_resource8.items()[:]:#just get idi
		count=np.array(dataDicCount[idi])
		xarr=dataDic_notall01_noInt[idi]
		x=np.concatenate((count,xarr))
		dataDic_01_count[idi]=x;#print x.shape
	save2pickle([dataDic_01_count,targetDic],'xy_dummy_Count_resource8')
