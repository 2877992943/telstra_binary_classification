...
def mapFreqLoc(locationArr,locDic):
	locFreq=np.array([locDic[loc] for loc in locationArr])
	return locFreq
...
if __name__=='__main__':
	...
	timeArr=uniqueInorder(trainTest['id'].values );print len(timeArr)
	timeArr=np.argsort(timeArr); 
	###count location
	locationArr=trainTest['location'].values #arr ['loc1',...]
	locDic=freq(locationArr)#{loc1:4times,...}
	print locDic.keys()[:2],locDic.values()[:2]
	locFreq=mapFreqLoc(locationArr,locDic);print locFreq.shape#arr
	#
	save2pickle([timeArr,locFreq],'timeInd_locFreq')
