...
if __name__=='__main__':
	...
	for idi in uniqueId[:]:
		...
		for col in patch.columns:
			fiStrList=uniqueInorder(patch[col].values)#['fea 1','fea 3'] 
		...
		dataDic[idi]=count.values()
		targetDic[idi]=target
		 
	#print dataDic,targetDic

	save2pickle([dataDic,targetDic,allFeaList],'dataTargetFeaAll_3')
	"""

	####get 01fea discrete_count fea
	dataDic01,ydic,_=load_pickle(dataPath+'dataTargetFeaAll_2')
	dataDicCount,_,_=load_pickle(dataPath+'dataTargetFeaAll_3')
	dataDic={}
	for idi,arr in dataDic01.items()[:]:
		count=np.array(dataDicCount[idi])
		x=np.concatenate((count,arr))
		dataDic[idi]=x;#print x.shape
	save2pickle([dataDic,ydic],'xy_dummy_discreteCount')
