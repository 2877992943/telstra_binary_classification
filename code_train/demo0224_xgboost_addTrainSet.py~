...
if __name__=='__main__':
	...
	dataDic,targetDic0=load_pickle(dataPath+'xy_dummy_Count')
	targetDic1=load_pickle(dataPath+'targetDic_half')
	#
	targetDicNew=targetDic0.copy()
	targetDicNew.update(targetDic1);print 'target',len(targetDicNew),len(targetDic0)
	# 
	##
	#split train set test set
	trainNewList=[];test0List=[];test0Id=[];targetNewList=[]
	 
	for idi,feaList in dataDic.items():
		##more trainset 
		if targetDicNew[idi] in [0,1,2]:# with label !=Nan
			trainNewList.append(feaList)
			targetNewList.append(targetDicNew[idi])
		###original testset
		if targetDic0[idi] not in [0,1,2]:
			test0List.append(feaList)
			test0Id.append(idi)
	
	 
	
	 
	print len(targetNewList),len(target0Id),len(targetDic0)-len(targetNewList)
	train=np.array(trainNewList)
	test=np.array(test0List)
	target=np.array(targetNewList)
	print train.shape,test.shape,target.shape
	save2pickle([train,test,target,test0Id],'trainTestArr_target_testId_2')
	...
	preds_out = pd.DataFrame({'id':test0Id,"predict_0": test_preds[:,0],"predict_1": test_preds[:,1],\
						"predict_2": test_preds[:,2]})
	preds_out.to_csv("../telstra_submission0228_3.csv")
