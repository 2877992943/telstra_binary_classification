...
def eval_wrapper(yhat, y):  #pred [n,3]  true[n,]
    if len(y.shape)!=2:
	dmmat=pd.get_dummies(np.array(y))
    	ytrue=dmmat.values #[n,3]
    if len(yhat.shape)!=2:
	dmmat=pd.get_dummies(np.array(yhat))
    	ypred=dmmat.values #[n,3]
    #ypred=yhat.reshape((-1,3)) #[n,3]
    #ytrue [0,1,2,1,0]->dummy
...
def predict2submit(clf,xgtest1,testId):
	################
	#test predict
	...
	preds_out.to_csv("../telstra_submission0226_2.csv")	 
...
if __name__=='__main__':
	...
	dataDic,targetDic=load_pickle(dataPath+'xy_dummy_discreteCount')
	...
	for boost_round in [50,1000][:1]:
		
		 
		for maxDepth in [7,32][:1]:#7  14
			...
			#
			#save2pickle(clf,'clf_'+str(maxDepth) )#not work,  parameter not present
			predict2submit(clf,xgtest1,testId)
	"""
	...
	predTrain=clf1.predict(xtrain)
	predTest=clf1.predict(xtest)
	 
	print 'rf',predTrain.shape,ytrain.shape
	print('Train err is:', eval_wrapper(predTrain, ytrain))# 32 100  err0.10
	print('Test err is(see whether overfit:', eval_wrapper(predTest, ytest))
	#for model combine
	#predTestModels.append(predTest)
